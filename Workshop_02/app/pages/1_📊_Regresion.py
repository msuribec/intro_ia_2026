...
st.markdown('<div class="page-title">📊 Regresión — Costos de Seguro Médico</div>',
            unsafe_allow_html=True)
st.markdown("**Dataset:** Medical Cost Insurance &nbsp;|&nbsp; **Target:** `charges` (USD)")
st.markdown("---")
...
with st.spinner("Cargando dataset de seguros médicos…"):
...
| One-Hot Encoding | `region` | 4 categorías → 3 dummies (drop_first=True) |
