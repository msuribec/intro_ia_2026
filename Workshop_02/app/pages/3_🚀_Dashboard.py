...
if is_regression:
    st.sidebar.markdown("**Medical Cost Insurance**\n- 1,338 registros\n- 8 features\n- Target: charges (USD)")
else:
    st.sidebar.markdown("**Digits (sklearn)**\n- 1,797 registros\n- 64 features (píxeles 8×8)\n- Target: dígito (0-9)")
...
with tab_ind:
    ...
    if is_regression:
        ...
        x_vec = np.array([[age, sex_enc, bmi, children, smoker_enc,
                           region_nw, region_se, region_sw]])

        if st.button("Predecir Costo Médico", type="primary"):
            pred_val = _predict_reg(model_obj, x_vec)
            st.markdown(f"""
            <div class="pred-box">
...
with tab_batch:
    ...
    if uploaded_file is not None:
        try:
            ...
            if is_regression:
                df_proc_up, _, _ = preprocess_insurance(df_upload.assign(charges=0))
                for col in feat_names:
                    if col not in df_proc_up.columns:
                        df_proc_up[col] = 0
                df_proc_up = df_proc_up[feat_names]
                x_batch = df_proc_up.values
                kind, m, sc = model_obj
