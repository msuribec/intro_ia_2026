...
st.markdown('<div class="page-title">📈 Clasificación — Reconocimiento de Dígitos (Digits)</div>',
            unsafe_allow_html=True)
st.markdown("**Dataset:** `sklearn.datasets.load_digits()` &nbsp;|&nbsp; "
...
with st.spinner("Cargando dataset Digits (sklearn)…"):
...
col4.metric("Balance de clases", balance_str)
...
tab_cls, tab_samples, tab_pix, tab_pca = st.tabs([
...
| Paso | Detalle |
|------|---------|
| Dataset origen | `sklearn.datasets.load_digits()` — 1,797 muestras, 64 features numéricas |
